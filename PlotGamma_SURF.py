...
def getGamma(t):
    vs = -LabVelocity(t, lat_SURF, lon_SURF)
...
for i in tqdm(range(Nvals)):
    vs = -LabVelocity(tvals[i], lat_SURF, lon_SURF)
    vs_hat = vs/np.sqrt(np.sum(vs**2))
...
for i in tqdm(range(Nvals)):
    vs = -LabVelocity(tvals2[i], lat_SURF, lon_SURF)
    vs_hat = vs/np.sqrt(np.sum(vs**2))
...
pl.fill_between(tvals-t0,(180-alpha_list)-(90-lat_SURF),180.0-np.abs(alpha_list - (90-lat_SURF)), color='b', alpha=0.4)
pl.plot(tvals-t0, (180-alpha_list)-(90-lat_SURF), 'b-')
pl.plot(tvals-t0, 180.0-np.abs(alpha_list - (90-lat_SURF)), 'b-')
...
pl.axhline(180-(90-lat_SURF), linestyle="--", color="k")
...
pl.plot([170.0, 195.0], [80.0, 14.0], 'k-', linewidth=0.75)
...
pl.savefig("plots/Gamma_SURF.pdf",bbox_inches="tight")
pl.show()