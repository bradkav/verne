...
v_th = DMU.vmin(10.0, 73.0, m_x=10.0)
...
def getVelDist(lsigstr, gamma_ind):
    ...
    gamma_vals1, vvals1, fvals1 = np.loadtxt("results/veldists/f_lmx1.0_lsig" + lsigstr + ".txt", unpack=True)
...
siglist = 10**np.asarray([-31.4, -31.2,-31.0,-30.8,-30.6, -30.4])
...
for i,sig in enumerate(siglist):
    ...
    #if (i == 0 or i == 10):
    #    labstr = str(i)
    ax1.plot(v, f, linewidth=2.0, color=col_list[i],label=str(int(sig*1e32)))
...
pl.text(425, 3e-1, r"$\sigma_p^\mathrm{SI} = 10^{-32} \,\,\mathrm{cm}^2 \times$", fontsize=18.0)
...
#pl.savefig('plots/SpeedDists_xsec_2.pdf', bbox_inches='tight')
pl.show()
