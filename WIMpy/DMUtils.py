...
def calcf_SHM(v, sigmav=156.0, vesc=533.0):
    ve = ROOT2*sigmav
    beta = ve/(sigmav**2)
    N1 = 1.0/(Nesc(sigmav, vesc)*sigmav**3*np.sqrt(2*np.pi))
    f = v*0.0
...
# Radon Transform (as a function of gamma)
def calcRT_full(v, theta, phi, gamma, sigmav=156.0, vesc=533.0):
    
    vlab = [-np.sin(gamma), 0, np.cos(gamma)]
    
    dotprod = (sin(theta)*cos(phi)*vlab[0]+ \
                sin(theta)*sin(phi)*vlab[1] + \
                cos(theta)*vlab[2])
    
    
    vhalo = (v + dotprod)
    prefactor = 1.0/(Nesc()*sigmav*np.sqrt(2.0*pi))
    RT = prefactor*(np.exp(-0.5*(vhalo/sigmav)**2.0)-np.exp(-0.5*(vesc/sigmav)**2.0))
 
    if hasattr(RT, "__len__"):
        RT[np.where(vhalo**2 > vesc**2)] = RT[np.where(vhalo**2 > vesc**2)]*0.0
    else:
        if (vhalo**2 > vesc**2):
            RT = 0
...
#---------------------------------------------------------
def calcEta(v,vlag=220.0, sigmav=156.0,vesc=544.0):
...
def calcSIFormFactor(E, m_N, old=False):
    ...
    #print 1e6*(0.1973)**2*2.0/(1.0*0.9315*m_N*(R1)**2)
    #print (1e6*3./2./m_N*0.038938)/(0.91*m_N**(0.33333)+0.3)**2
