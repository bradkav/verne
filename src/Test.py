...
fv_file = "../results/veldists/f_SI_surface_mx0.500_lsig-40.00.txt"

#Check whether the test velocity distribution exists, 
#otherwise, generate it
if  (os.path.isfile(fv_file) != True):
    os.system("python3 CalcVelDist.py -m_x 0.5 -sigma_p 1e-40 -loc surface -interaction SI")
...
for i in range(N_gamma):
    ...
    vvals,fvals = getVelDist(0.5, 1e-40, gamma_ind=i)
...
if (WIMpy_installed):
    ...
    dRdE_WIMpy_verne = WIMpy.DMUtils.dRdE_generic(E_list, N_p=14, N_n=14, m_x=0.5, sig=1e-40, eta_func=eta_interp)
...
plt.title(r"$m_\chi = 500 \,\mathrm{MeV}; \sigma_p^{\mathrm{SI}} = 10^{-40}\,\mathrm{cm}^2$; $\gamma = \pi$; Silicon target")
